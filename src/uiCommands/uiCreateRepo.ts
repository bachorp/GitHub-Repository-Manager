...
export type OnRepositoryCreation = (
 ...
 */
export const uiCreateRepoCore = async (options: {
    ...
    try {
        ...
        if (orgsUserCanCreateRepo.length === 1) {
            ...
            const pick = await myQuickPick({
                items: orgsUserCanCreateRepo.map((e) => ({
                    ...
                    description:
                        e.login === User.login ? userDescription : e.login,
                })),
...
/** Doesn't throw errors. */
export const uiCreateRepo = async (): Promise<void> => {
                ...
                (async () => {
                    ...
                    if (answer === actions[0]) {
                        if (!newRepository.owner.login) {
                            ...
                            throw new Error(
                                `newRepository.owner?.login doesn't exist!`,
                            )
                        }
                        await uiCloneTo({
