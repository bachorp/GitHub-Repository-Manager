...
const getIcon = (repo: Repository): vscode.ThemeIcon | undefined => {
    const args = ((): { name: string; color?: string } => {
        if (hasRepoRemoteWithUserAccess(repo)) {
            if (!isRepoOnDisk(repo)) {
                return {
                    name: 'source-control',
                    color: 'githubRepositoryManager.remote',
                }
            }
            if (repo.isPrivate) {
                return {
                    name: 'lock',
                    color: 'githubRepositoryManager.private',
                }
            }
            if (repo.isFork) {
                return {
                    name: 'repo-forked',
                    color: 'githubRepositoryManager.fork',
                }
            }
...
const prettifyLocalPath = (path: string): string =>
    path.replace(RegExp(`^${os.homedir()}`), '~')

// the | &nbsp; | adds a little more spacing.
const getTooltip = (repo: Repository) => {
    const repoWithUserAccess = hasRepoRemoteWithUserAccess(repo)
        ? repo
        : undefined
    const R = (
        fun: (f: Repository<boolean, 'user-is-member'>) => string,
    ): string => (repoWithUserAccess ? fun(repoWithUserAccess) : '')
...
|     |     |     |
| --- | --- | --- |
**Name** | &nbsp; | ${repo.name}${R((r) => `\r\n**Description** | &nbsp; | ${r.description || 'No description'}`)}${R((r) => `\r\n**Author** | &nbsp; | ${r.ownerLogin}`)}${R((r) => `\r\n**Visibility** | &nbsp; | ${r.isPrivate ? 'Private' : 'Public'}`)}${R((r) => (r.languageName ? `\r\n**Language** | &nbsp; |${r.languageName}` : ''))}${R(
        (r) =>
            r.isFork
                ? `\r\n**Fork of** | &nbsp; | ${r.parentRepoOwnerLogin} / ${r.parentRepoName}`
                : '',
    )}${R((r) => `\r\n**Updated at** | &nbsp; | ${r.updatedAt.toLocaleString()}`)}${R((r) => `\r\n**Created at** | &nbsp; | ${r.createdAt.toLocaleString()}`)}${D((r) => `\r\n**Local path** | &nbsp; | ${prettifyLocalPath(r.localPath)}`)}${D(
        (r) =>
            r.dirty !== 'clean'
