...
export const getOrgRepos = async ({
    ...
    login: string
    octokit: Octokit
}): Promise<Array<Repository<false, 'user-is-member'>>> => {
    try {
        const repos: Array<Repository<false, 'user-is-member'>> = []
        ...
        do {
            ...
            const { nodes, pageInfo } =
                response.viewer.organization.repositories

            ;({ endCursor, hasNextPage } = pageInfo)
            repos.push(
                ...nodes.map((node: any) => extractRepositoryFromData(node)),
            )
        } while (hasNextPage)
